...
const PLANS = [
  { id:'premium_monthly',  name:'Premium',          price:249,   period:'aylık',  months:1,  color:'#C9832E', badge:'En Popüler',
    feats:['5 aktif ilan / ay','5 pet pasaport','Telefon & konum görme','Tüm ilanları görme','AI veteriner asistanı','Mesajlaşma','Öncelikli destek'] },
  { id:'premium_yearly',   name:'Pro Yıllık',        price:2490,  period:'yıllık', months:12, color:'#1D9E75', badge:'%17 İndirim',
    feats:["Premium'un tümü",'20 ilan/ay (günde 5)','20 pet pasaport','Topluluk kurabilir','Blog yazabilir','Özel rozet'] },
  { id:'corporate_monthly',name:'Kurumsal Aylık',    price:599,   period:'aylık',  months:1,  color:'#534AB7', badge:'Veteriner & İşletme',
    feats:['Profil sayfası aktif','Doğrudan iletişim','Randevu sistemi','Soru-cevap modülü','Arama önceliği','Onaylı İşletme rozeti'] },
  { id:'corporate_yearly', name:'Kurumsal Yıllık',   price:6110,  period:'yıllık', months:12, color:'#3C3489', badge:'%15 İndirim',
    feats:["Kurumsal aylığın tümü",'Aylık performans raporu','Öncelikli listeleme','Toplu ödeme avantajı','7/24 destek','Özel hesap yöneticisi'] },
];
...
export default function PremiumPage() {
  ...
  const planExpiryRaw = userPlan?.planExpiry;
  const planExpiry   = planExpiryRaw
    ? (typeof planExpiryRaw.toDate === 'function' ? planExpiryRaw.toDate() : new Date(planExpiryRaw))
    : null;
          ...
          {isPremium && (
            <div className="bg-white rounded-[20px] border border-[rgba(196,169,107,.12)] p-5 mb-6 flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="font-semibold text-[#2F2622]">Mevcut Plan: {userPlan.planName || userPlan.plan}</div>
                {planExpiry && (
                  <div className="text-sm text-[#7A7368] mt-1">
                    {daysLeft > 0 ? `${daysLeft} gün kaldı · ${planExpiry.toLocaleDateString('tr-TR')}` : 'Planınız sona erdi'}
                  </div>
                )}
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-5 mb-6">
                {PLANS.map(plan => (
                  <div key={plan.id}
                    ...
                    <button disabled={codeLoading}
                      className="w-full py-3 rounded-full text-white text-sm font-semibold hover:opacity-90 disabled:opacity-60"
            ...
            <div className="max-w-[520px] mx-auto">
              ...
              <div className="bg-white rounded-[24px] border border-[rgba(196,169,107,.12)] p-6">
                <div className="text-center mb-6">
                  ...
                  <h2 className="font-serif text-2xl font-semibold text-[#2F2622] mb-1">{selPlan.name}</h2>
                ...
                <div className="space-y-3 mb-5">
                  {[
                    {n:1, t:'Kodu kopyalayın',           d:'Yukarıdaki kodu panoya kopyalayın'},
                    {n:2, t:'Shopier\'e gidin',           d:`Aşağıdaki butona tıklayın ve ödemeyi tamamlayın`},
                    {n:3, t:'Sipariş notuna kodu yazın',  d:`"Sipariş notu" alanına ${myCode} yazın`},
                    {n:4, t:'Onay bekleyin',              d:'Para geçtikten sonra premium aktif edilir'},
                    {n:5, t:'Mail gelecek',               d:'Aktivasyon onay maili adresinize gönderilir'},
                  ].map(s => (
                ...
                <div className="bg-[rgba(201,131,46,.06)] border border-[rgba(201,131,46,.2)] rounded-[12px] p-3 text-xs text-[#8B7355] text-center">
                  Sipariş notuna <strong>{myCode}</strong> yazmayı unutmayın!
                </div>
              </div>
              <div className="mt-4 bg-white rounded-[20px] border border-[rgba(196,169,107,.12)] p-4 text-center">
                <div className="text-sm text-[#7A7368] mb-2">Ödemeyi tamamladıysanız:</div>
                <button onClick={checkActivation} className="text-sm text-[#C9832E] hover:underline font-medium">
