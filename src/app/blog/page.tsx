  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await addDoc(collection(db, 'blogSubmissions'), {
        name: formName, email: formEmail, title: formTitle,
        category: formCat, content: formBody,
    ...
    <>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-[6px] z-[800] flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] w-full max-w-[600px] p-6 sm:p-8 shadow-xl max-h-[90vh] overflow-y-auto relative">
            <button onClick={() => { setShowForm(false); setSent(false); }} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-[#F7F2EA] flex items-center justify-center text-[#7A7368] hover:bg-[#EDE5D3]">✕</button>
            {sent ? (
              <div className="text-center py-6">
                <div className="text-5xl mb-4">🎉</div>
                <h3 className="font-serif text-2xl font-semibold text-[#2F2622] mb-2">İçeriğiniz Alındı!</h3>
                <p className="text-sm text-[#7A7368] mb-5">Editör ekibimiz inceledikten sonra yayına alacak.</p>
                <button onClick={() => { setShowForm(false); setSent(false); }} className="bg-[#C9832E] text-white text-sm font-semibold px-5 py-2 rounded-full hover:bg-[#b87523]">Kapat</button>
              </div>
            ) : (
              <>
        ...
        <section className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-16 py-10">
          {activeCat === 'Tümü' && (
            <Link href={`/blog/${featured.slug}`}
              className="grid lg:grid-cols-[1.2fr_1fr] rounded-[24px] overflow-hidden border border-[rgba(201,131,46,.1)] hover:shadow-xl hover:-translate-y-1 transition-all mb-10 block bg-white">
